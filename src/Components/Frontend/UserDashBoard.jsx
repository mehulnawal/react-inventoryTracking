    ...
    return (
        <>
            <div className={`p-4 sm:p-6 max-w-screen-xl mx-auto ${theme === 'dark' ? 'bg-gray-900' : 'bg-gray-50'} min-h-screen`}>
                <h2 className={`text-2xl sm:text-3xl font-bold ${theme === 'dark' ? 'text-white' : 'text-gray-900'} mb-8`}>Dashboard</h2>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-8">
                    ...
                    <div className={`${theme === 'dark' ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} rounded-xl shadow-sm border p-4 sm:p-6`}>
                        <div className="flex items-center justify-between mb-6">
                            <h3 className={`text-lg sm:text-xl font-semibold ${theme === 'dark' ? 'text-white' : 'text-gray-900'} flex items-center`}>
                        ...
                        <div className="space-y-4 max-h-[400px] overflow-y-auto">
                            {Object.entries(transaction).length > 0 ? (
                                lowStock.map(([id, value]) => (
                                    <div
                                        key={id}
                                        className={`flex items-center p-3 sm:p-4 ${theme === 'dark' ? 'bg-gradient-to-r from-red-900/20 to-orange-900/20 border-red-800' : 'bg-gradient-to-r from-red-50 to-orange-50 border-red-200'} border rounded-lg hover:shadow-md transition-all duration-200`}
                                    >
                                        {/* Product Image */}
                                        <div className="flex-shrink-0 mr-3 sm:mr-4">
                                            <div className="relative">
                                                <img
                                                    ...
                                                    alt={value.category}
                                                    className={`w-12 h-12 sm:w-16 sm:h-16 rounded-lg object-cover border-2 ${theme === 'dark' ? 'border-red-700' : 'border-red-200'}`}
                                                />
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="flex items-center justify-between">
                                                <h4 className={`text-sm sm:text-base font-semibold truncate ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>
                                                    {value.category}
                                                </h4>
                                                <span className={`ml-2 inline-flex items-center px-2.5 py-0.5 rounded-full text-xs sm:text-sm font-medium ${theme === 'dark' ? 'bg-red-900/50 text-red-300' : 'bg-red-100 text-red-800'}`}>
                                                    {value.quantity <= 10 && value.quantity > 5 ? 'Low' : value.quantity <= 5 && value.quantity > 0 ? 'Critical' : 'Out of Stock'}
                                                </span>
                                            </div>

                                            <div className={`mt-1 sm:mt-2 flex items-center text-xs sm:text-sm ${theme === 'dark' ? 'text-gray-400' : 'text-gray-600'}`}>
                                ...
                                <div className="text-center py-10">
                                    <div className="flex flex-col items-center">
                                        <div className={`w-14 h-14 sm:w-16 sm:h-16 ${theme === 'dark' ? 'bg-green-900/20' : 'bg-green-100'} rounded-full flex items-center justify-center mb-4`}>
                                        ...
                                        <p className={`${theme === 'dark' ? 'text-gray-400' : 'text-gray-500'} text-sm max-w-xs`}>
                    ...
                    {/* Transaction Form */}
                    <div className={`${theme === 'dark' ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} rounded-xl shadow-sm border p-4 sm:p-6`}>
                        <h3 className={`text-lg sm:text-xl font-semibold ${theme === 'dark' ? 'text-white' : 'text-gray-900'} mb-6`}>Record Transaction</h3>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        className={`w-full border ${theme === 'dark' ? 'border-gray-600 bg-gray-700 text-white' : 'border-gray-300 bg-white text-gray-900'} rounded-lg px-4 py-3`}
                                    />
                                    <span className='text-red-500'>{error?.date}</span>
                                </div>
                            </div>
                ...
                <div className={`mt-8 rounded-xl shadow-sm border p-4 sm:p-6 ${theme === 'dark' ? 'bg-gray-800 border-gray-700 text-white' : 'bg-white text-gray-900 border-gray-200'}`}>
                    ...
                    <div className="overflow-x-auto">
                        <table className={`w-full min-w-[600px] ${theme === 'dark' ? 'bg-gray-800' : 'bg-white'} rounded-lg shadow-lg overflow-hidden`}>
                            <thead className={`${theme === 'dark' ? 'bg-gray-700' : 'bg-gray-100'}`}>
                                <tr>
                                    <th className={`px-4 sm:px-6 py-3 text-left text-xs sm:text-sm font-semibold ${theme === 'dark' ? 'text-gray-300' : 'text-gray-600'} uppercase tracking-wider`}>
                                        Type
                                    </th>
                                    <th className={`px-4 sm:px-6 py-3 text-left text-xs sm:text-sm font-semibold ${theme === 'dark' ? 'text-gray-300' : 'text-gray-600'} uppercase tracking-wider`}>
                                        Category
                                        ...
                                        {Object.entries(transaction).map(([id, data]) => (
                                            ...
                                            >
                                                ...
                                                <td className="px-4 sm:px-6 py-3 whitespace-nowrap">
                                                    <span
                                                        className={`inline-flex items-center px-3 py-1 rounded-full text-sm sm:text-md ${data.type === 'sale'
                                                            ? theme === 'dark'
                                                ...
                                                {/* category */}
                                                <td className={`px-4 sm:px-6 py-3 whitespace-nowrap text-sm sm:text-md font-medium ${theme === 'dark' ? 'text-gray-200' : 'text-gray-900'}`}>
                                                    {data.category}
                                                </td>

                                                {/* quantity */}
                                                <td className={`px-4 sm:px-6 py-3 whitespace-nowrap text-xs sm:text-sm ${theme === 'dark' ? 'text-gray-300' : 'text-gray-600'}`}>
                                                ...
                                                {/* date */}
                                                <td className={`px-4 sm:px-6 py-3 whitespace-nowrap text-sm ${theme === 'dark' ? 'text-gray-400' : 'text-gray-500'}`}>
